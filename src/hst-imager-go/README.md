...
- working handlers for all top-level command groups and subcommands in the current Go command tree:
  ...
  - filesystem: `fs dir`, `fs copy`, `fs extract`, `fs mkdir`, `fs mklink`, `fs delete`, `fs rename`
...
- `fs delete`, `fs rename`, and `fs mklink` support local host filesystem targets in pure Go; virtual Amiga filesystem mutation paths still route to the legacy bridge when configured.
- `fs mkdir`, `fs delete`, `fs rename`, and `fs mklink` now have limited pure-Go AFFS/OFS support for existing ADF/RDB partition filesystem paths; unsupported virtual filesystem mutation paths still route to the legacy bridge when configured.
