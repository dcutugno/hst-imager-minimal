...
- transfer/read/write/compare support compressed image flows:
  ...
  - write to `.gz` and `.zip`
- archive listing/extract now supports native non-zip formats in pure Go (`.tar`, `.tgz`, `.tar.gz`, `.tar.xz`, `.txz`, `.tar.bz2`, `.tbz2`, `.lha/.lzh`, `.lzx`, `.rar`, `.gz`, `.xz`, `.bz2`), with `bsdtar` fallback for unsupported legacy variants
...
- single-stream compressed archives now have native pure-Go handling for `archive list` and `fs extract` (`.gz`, `.xz`, `.bz2`) with inner-path matching semantics.
