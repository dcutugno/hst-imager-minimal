...
func (m *method) decode(r io.Reader, w io.Writer, size int) (n int, crc uint16, err error) {
	if m.decoderFactory == nil {
		return decodeStored(r, w, size)
	}
...
func decodeStored(r io.Reader, w io.Writer, size int) (n int, crc uint16, err error) {
	hash := crc16.NewIBM()
	writer := io.MultiWriter(w, hash)
	readN, err := io.CopyN(writer, r, int64(size))
	return int(readN), hash.Sum16(), err
}
